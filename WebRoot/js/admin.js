...
function packageList(){
	$("#tclb").click(function(){
		$.post("packageAction_list",function(data,status){
			var tcContent="";
			$(".pageContent").html(tcContent);
			tcContent+="<div class='wrapContent'>";
			for(i=0;i<data.length;i++){
				tcContent+="<div class='leftImage'><img src='./"+data[i].pictureAddress+"'/></div>";
				tcContent+="<div class='rightContent'>";
				tcContent+="<div><i>原价：</i><span>"+data[i].orginalPrice+"</span></div>";
				tcContent+="<div><i>现价：</i><span>"+data[i].currentPrice+"</span></div>";
				tcContent+=" <div><i>详细介绍：</i><span>"+data[i].detailInfo+"</span></div> ";
				tcContent+="<div><i><a href='#' onclick='openwin();'>修改套餐</a></i><a href='#'>删除套餐</a></i></div></div>";
			}
		    	tcContent+="</div>";
			$(".pageContent").css({"width":"600px","float":"left","border":"1px solid #ccc"});
			$(".pageContent").html(tcContent);
		},"json");
	})
}
...
function tcSubmit(){
		  ...
		  var tcphotoUrl=$("#photoUrl").val();
			$(".pageContent").html(tcContent);
			$.post("packageAction_add",{price:tcprice,info:tcinfo,photoUrl:tcphotoUrl},function(data,status){
				if(data=="success"){
...
function openwin() 
{ 
OpenWindow=window.open("","_blank","height=350,width=450,top=300,left=500"); 
// 写成一行
