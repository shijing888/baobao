	...
	function commonCalContxt(current_page, page_size) {
		var path = "commentByPage?page=" + current_page + "&pagesize="
				+ page_size;
	...
	function calCount(json) {
		var countpage = s_recordsize % pagesize == 0 ? s_recordsize
				/ pagesize : Math.ceil(s_recordsize / pagesize);
	...
	function processObj(json) {
		...
		for (i = 0; i < json[0].length; i++) {
			...
			contable.innerHTML += "<tr height='18' bgcolor='#FFFFFF' id='"+ i+"'>"
					...
					+"<input type='button' value='通过' class='checkbutton' onclick='passrecord(this,"
					+ temp + ")'/>"
